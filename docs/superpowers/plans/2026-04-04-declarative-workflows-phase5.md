# Declarative Workflows — Phase 5: CLI Commands, MCP Tool & E2E Tests

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `tusk workflow list` and `tusk workflow info <name>` CLI commands, add `tusk_workflow_list` MCP tool, update MCP config validation, add E2E tests, and mark the initiative complete.

**Architecture:** New CLI commands delegate to `WorkflowService`. MCP tool uses the existing `addTool` pattern. E2E tests use the existing harness. Each task is an independent commit.

**Tech Stack:** Go, Cobra (CLI), mcp-go (MCP)

**Prerequisites:** Phase 4 must be complete (final WorkflowService API in place).

---

### Task 1: Add `tusk_workflow_list` MCP tool

Register the new tool and implement its handler. Update MCP config validation.

**Files:**
- Modify: `internal/mcp/server.go` (tool registration + config validation)
- Modify: `internal/mcp/tools.go` (handler + response type)

- [ ] **Step 1: Add tool registration in `internal/mcp/server.go` `registerTools`**

Add at the end of `registerTools`, after the `tusk_task_tree` block:
...
`transitionResponse` is already defined in `resources.go`.
...
Add `tusk workflow list` and `tusk workflow info <name>` with text and JSON output.
...
Add at the end of the file. Note: `strings`, `io`, `fmt`, `json`, and `domain` are already imported in this file.
...
- [ ] **Step 1: Mark Declarative Workflows stories complete**

In `ROADMAP.md`, under `### Initiative: Declarative Workflows` (lines 239-266), change all `- [ ]` to `- [x]`.
