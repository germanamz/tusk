...
- `Sync.ApplyFile(ctx, fileRow, units)` — diffs the parsed units against existing rows **by address**, inserts/deletes/updates rows, rewrites `contains` edges, and enqueues embeds for new or content-changed leaves (sections are never enqueued).
...
- **Section addresses are positional and independent of heading text.** Editing a heading keeps its address (and every descendant address); only the section row's title/text and `content_hash` change.
...
Each sub-unit row stores `content_hash` = sha256 of the **embed payload** (for leaves — a table cell's payload is `"<column-header>: <cell-text>"`), or sha256 of the heading for sections (sections are never embedded). It drives the sync re-embed decision and is the key the embedding store dedupes on: identical content anywhere in the workspace is embedded once and shared.
...
- address only in the new set → insert the row; enqueue an embed (leaves only).
- address in both, `content_hash` changed → update the row; re-enqueue the embed.
- address in both, unchanged → leave the row untouched.
- address only in the old set → delete the row (its `contains` edge and embedding mapping cascade away).
