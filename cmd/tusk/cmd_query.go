...
the document is restructured.`,
  ...
  tusk query 'type=note' --semantic 'auth flow' --json --take 1 \
	...
	queryCmd.Flags().IntVar(&hops, "hops", 0, "graph-expansion BFS depth (1 or 2; omit to inherit manifest)")
	queryCmd.Flags().Float64Var(&graphWeight, "graph-weight", -1, "per-hop weight applied to expanded candidates in [0,1]; omit to inherit manifest")
	// -1 is the "unset" sentinel (a real value must be in [0,1]); show it as
	// "inherit" so the help does not advertise a passable negative default.
	queryCmd.Flags().Lookup("graph-weight").DefValue = "inherit"
