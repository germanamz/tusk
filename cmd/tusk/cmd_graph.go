...
legend. Its size and brightness both grow with its in-degree (the number
of incoming edges), so the most-referenced nodes stand out as large,
bright hubs while leaf nodes recede. Use the facet bar to filter by type
or edge kind, click a node to inspect it and highlight its neighbors,
and drag to rotate, alt+drag to pan, scroll to zoom.
