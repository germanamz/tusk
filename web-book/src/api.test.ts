...
describe('fetchIndex', () => {
  it('GETs the relative index endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('./api/index', expect.anything())
...
describe('fetchNode', () => {
  it('percent-encodes the id and preserves "/" separators', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      './api/node/notes/' + encodeURIComponent('c#S1P1'),
      expect.anything(),
    )
  })
...
describe('postSearch', () => {
  ...
  it('POSTs the JSON body to the relative search endpoint', async () => {
...
describe('fetchRelated', () => {
  it('omits hops/weight/edge_types entirely when unspecified', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('./api/related/notes/a', expect.anything())
  })
