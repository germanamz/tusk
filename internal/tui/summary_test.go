...
func TestResolveSummaryMode(test *testing.T) {
	cases := []struct {
		name     string
		args     []string
...
func TestComputeTotals_Empty(test *testing.T) {
	got := computeTotals(nil)
	if got == nil {
		test.Fatal("computeTotals(nil) must return non-nil")
	}
	if got.Done != 0 || got.Total != 0 || got.Percent != 0 {
		test.Fatalf("zero rollup expected, got %+v", got)
	}
	if got.StatusCounts == nil {
		test.Fatal("StatusCounts must be a non-nil empty slice")
	}
	if len(got.StatusCounts) != 0 {
		test.Fatalf("expected zero buckets, got %d", len(got.StatusCounts))
	}
}

func TestComputeTotals_SingleBlock(test *testing.T) {
	...
	if got.Done != 2 || got.Total != 5 {
		test.Fatalf("counts: got Done=%d Total=%d", got.Done, got.Total)
	}
	if got.Percent != 0.4 {
		test.Fatalf("percent: got %v", got.Percent)
	}
	if len(got.StatusCounts) != 2 || got.StatusCounts[0].Name != "pending" || got.StatusCounts[1].Name != "completed" {
		test.Fatalf("buckets out of order: %+v", got.StatusCounts)
	}
}

func TestComputeTotals_MergeBuckets(test *testing.T) {
	...
	if got.Done != 3 || got.Total != 7 {
		test.Fatalf("totals: got Done=%d Total=%d, want 3 and 7", got.Done, got.Total)
	}
	wantPercent := float64(3) / float64(7)
	if got.Percent != wantPercent {
		test.Fatalf("percent: got %v want %v", got.Percent, wantPercent)
	}

	wantNames := []string{"pending", "completed", "active"}
	if len(got.StatusCounts) != len(wantNames) {
		test.Fatalf("bucket count: got %d want %d", len(got.StatusCounts), len(wantNames))
	}
	for i, want := range wantNames {
		if got.StatusCounts[i].Name != want {
			test.Fatalf("bucket %d: got %q want %q", i, got.StatusCounts[i].Name, want)
		}
	}
	wantCounts := map[string]int{"pending": 2, "active": 2, "completed": 3}
	for _, statusCount := range got.StatusCounts {
		if statusCount.Count != wantCounts[statusCount.Name] {
			test.Fatalf("bucket %s: got count %d want %d", statusCount.Name, statusCount.Count, wantCounts[statusCount.Name])
		}
	}
}

func TestComputeTotals_ZeroPercentWhenNoTotal(test *testing.T) {
	blocks := []*domain.SummaryBlock{
		{Rollup: domain.Rollup{Done: 0, Total: 0, StatusCounts: []domain.StatusCount{}}},
	}
	got := computeTotals(blocks)
	if got.Percent != 0 {
		test.Fatalf("percent: got %v want 0", got.Percent)
	}
	if got.StatusCounts == nil {
		test.Fatal("StatusCounts must be non-nil")
	}
}

func TestRenderBlockText_Branch(test *testing.T) {
	taskID := uuid.New()
	level := "milestone"
	block := &domain.SummaryBlock{
		Task: &domain.Task{
	...
	renderer := NewRenderer(nil, "text", false, nil)
	var buf bytes.Buffer
	if err := renderBlockText(&buf, renderer, block); err != nil {
		test.Fatalf("renderBlockText: %v", err)
	}
	got := buf.String()
	...
	if got != want {
		test.Fatalf("renderBlockText output mismatch\n--- got ---\n%s--- want ---\n%s", got, want)
	}
}

func TestRenderBlockText_LeafNoLevel(test *testing.T) {
	block := &domain.SummaryBlock{
		Task: &domain.Task{
			ShortID: "deadbeef",
			Title:   "Plain task",
	...
	if strings.Contains(got, "level:") {
		test.Fatalf("expected no level: line, got:\n%s", got)
	}
	if strings.Contains(got, "breakdown:") {
		test.Fatalf("expected no breakdown: line for empty rollup, got:\n%s", got)
	}
	if !strings.Contains(got, "0/0 done, –%") {
		test.Fatalf("expected empty progress dash, got:\n%s", got)
	}
}

func TestRenderTotalsText_Basic(test *testing.T) {
	...
	renderer := NewRenderer(nil, "text", false, nil)
	var buf bytes.Buffer
	if err := renderTotalsText(&buf, renderer, totals); err != nil {
		test.Fatalf("renderTotalsText: %v", err)
	}
	got := buf.String()
	want := "TOTALS    7/9 done, 78%\n" +
		"          pending: 1, active: 1, completed: 7\n"
	if got != want {
		test.Fatalf("renderTotalsText output mismatch\n--- got ---\n%s--- want ---\n%s", got, want)
	}
}
