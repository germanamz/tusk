...
func TestDecode_SchemaVersionMismatch(t *testing.T) {
	...
	if len(importErr.Issues) != 1 {
		t.Fatalf("expected 1 issue, got %d", len(importErr.Issues))
	}
	issue := importErr.Issues[0]
	if issue.Kind != "schema" {
		t.Errorf("expected Kind=schema, got %q", issue.Kind)
	}
	if !strings.Contains(issue.Message, "999") {
		t.Errorf("expected message to mention 999, got %q", issue.Message)
	}
	if !strings.Contains(issue.Message, "1") {
		t.Errorf("expected message to mention supported version 1, got %q", issue.Message)
	}
}
...
func TestDecode_MalformedJSON(t *testing.T) {
	...
	if importErr.Issues[0].Kind != "json" {
		t.Errorf("expected Kind=json, got %q", importErr.Issues[0].Kind)
	}
}
...
func TestDecode_UnknownTopLevelField(t *testing.T) {
	...
	if importErr.Issues[0].Kind != "json" {
		t.Errorf("expected Kind=json (DisallowUnknownFields surfaces as a JSON decode error), got %q", importErr.Issues[0].Kind)
	}
	if !strings.Contains(importErr.Issues[0].Message, "foo") {
		t.Errorf("expected message to name the unknown field 'foo', got %q", importErr.Issues[0].Message)
	}
}

func TestDecode_EmptyWorkspace(t *testing.T) {
	...
	if got.SchemaVersion != SchemaVersion {
		t.Errorf("SchemaVersion mismatch: got %d", got.SchemaVersion)
	}
	if got.TuskVersion != "v0.13.0" {
		t.Errorf("TuskVersion mismatch: got %q", got.TuskVersion)
	}
	if len(got.Tasks) != 0 || len(got.Projects) != 0 || len(got.Workflows) != 0 ||
		...
		t.Errorf("expected all entity lists empty, got %+v", got)
	}
}

func TestImportError_ErrorMessage(t *testing.T) {
	e := &ImportError{Issues: []ImportIssue{
