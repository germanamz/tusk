...
func TestRead_AbsentReturnsZero(test *testing.T) {
	...
	if got != 0 {
		test.Fatalf("expected 0 for absent manifest epoch, got %d", got)
	}
}
...
func TestRead_ParsesExistingValue(test *testing.T) {
	...
	if writeErr := os.WriteFile(filepath.Join(root, ".tusk", ManifestEpochFilename), []byte("42\n"), 0o644); writeErr != nil {
...
func TestBump_LeavesNoTempFile(test *testing.T) {
	...
	for _, entry := range entries {
		if entry.Name() != ManifestEpochFilename {
