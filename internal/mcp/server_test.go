...
func TestRegisterTools_FiltersDisabledTools(t *testing.T) {
	...
	if len(full.toolGroups) != 12 {
		t.Errorf("full server: expected 12 tools, got %d", len(full.toolGroups))
	}
	if len(filtered.toolGroups) != 10 {
		t.Errorf("filtered server: expected 10 tools (relation group disabled), got %d", len(filtered.toolGroups))
	}
}
