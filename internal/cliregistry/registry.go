...
type VerbSpec struct {
	...
	Positionals []string
	// Tool is the MCP tool name that exposes the same operation (e.g.
	// "tusk_node_list"). Set for every verb — read or write — that has an
	// MCP counterpart, so the CLI<->MCP parity test can assert it is
	// registered. Aliasability is governed by ReadOnly membership, not by
	// this field. Empty only for verbs with no MCP tool.
	Tool string
...
var Write = map[string]VerbSpec{
	"node create": {Tool: "tusk_node_create", Verb: "node create", ReadOnly: false},
	"node modify": {Tool: "tusk_node_modify", Verb: "node modify", ReadOnly: false},
	"node move":   {Tool: "tusk_node_move", Verb: "node move", ReadOnly: false},
	"node delete": {Tool: "tusk_node_delete", Verb: "node delete", ReadOnly: false},
	"edge add":    {Tool: "tusk_edge_add", Verb: "edge add", ReadOnly: false},
	"edge remove": {Tool: "tusk_edge_remove", Verb: "edge remove", ReadOnly: false},
}
