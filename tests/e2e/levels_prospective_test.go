...
func TestLevelsProspective(t *testing.T) {
	scenarios := []Scenario{
		{
			...
			Steps: []Step{
				...
				},
				{
					...
					Assert: func(t *testing.T, r Result) {
						t.Helper()
						if !strings.Contains(r.Stdout, "$0.short_id") && !strings.Contains(r.Stdout, "missing") {
							// short_id expansion happens before assert; just require "missing" reason.
							if !strings.Contains(r.Stdout, "missing") {
								t.Fatalf("expected missing reason in output, got: %q", r.Stdout)
							}
						}
					},
				},
