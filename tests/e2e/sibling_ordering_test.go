...
func assertOrderField(test *testing.T, parsed any, want float64) {
	test.Helper()
	mapped, ok := parsed.(map[string]any)
	if !ok {
		test.Fatalf("assertOrderField: expected map, got %T", parsed)
	}
	got, present := mapped["order"]
	if !present {
		test.Fatalf("assertOrderField: order field missing in %v", mapped)
	}
	gotFloat, ok := got.(float64)
	if !ok {
		test.Fatalf("assertOrderField: expected float64, got %T (%v)", got, got)
	}
	if gotFloat != want {
		test.Fatalf("assertOrderField: got %v, want %v", gotFloat, want)
	}
}

// assertOrderAbsent asserts the "order" field is absent or explicitly nil.
func assertOrderAbsent(test *testing.T, parsed any) {
	test.Helper()
	mapped, ok := parsed.(map[string]any)
	if !ok {
		test.Fatalf("assertOrderAbsent: expected map, got %T", parsed)
	}
	if v, present := mapped["order"]; present && v != nil {
		test.Fatalf("assertOrderAbsent: expected absent/nil, got %v", v)
	}
}

func TestSiblingOrdering(test *testing.T) {
	scenarios := []Scenario{
		{
			...
			Steps: []Step{
				...
				{Args: []string{"task", "create", "Child C", "parent=$0.short_id"}}, // 3
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						test.Helper()
						assertOrderField(test, parsed, 1.0)
					},
					AssertText: func(test *testing.T, output string) {
						test.Helper()
						assertContains(test, output, "Order:")
					},
				},
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						test.Helper()
						assertOrderField(test, parsed, 2.0)
					},
				},
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						test.Helper()
						assertOrderField(test, parsed, 3.0)
					},
				},
			},
		},
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						for index := range want {
							if titles[index] != want[index] {
								test.Fatalf("tree position %d: got %q want %q", index, titles[index], want[index])
							}
						}
					},
				},
			...
			Steps: []Step{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						for index := range want {
							if got[index] != want[index] {
								test.Fatalf("post-move position %d: got %q want %q", index, got[index], want[index])
							}
						}
					},
				},
				// The moved task's order should now be strictly between A (1.0) and B (2.0).
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						v, ok := mapped["order"].(float64)
						if !ok {
							test.Fatalf("expected numeric order, got %v", mapped["order"])
						}
						if !(v > 1.0 && v < 2.0) {
							test.Fatalf("expected midpoint in (1.0, 2.0), got %v", v)
						}
					},
				},
			},
			...
			Steps: []Step{
				...
				{Args: []string{"task", "move", "$2.short_id", "--first"}},          // 4
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						for index := range want {
							if titles[index] != want[index] {
								test.Fatalf("position %d: got %q want %q", index, titles[index], want[index])
							}
						}
					},
				},
				// Moved task's order should be less than the current head's previous order.
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						if v >= 1.0 {
							test.Fatalf("expected order < 1.0 after --first, got %v", v)
						}
					},
				},
			},
				...
				{Args: []string{"task", "move", "$2.short_id", "--after", "$3.short_id"}}, // 4
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						if mapped["parent_id"] == nil || mapped["parent_id"] == "" {
							test.Fatal("expected parent_id to be set after cross-parent move")
						}
					},
				},
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						children := root["children"].([]any)
						if len(children) != 2 {
							test.Fatalf("expected 2 children under Parent B, got %d", len(children))
						}
						...
						for index := range want {
							if titles[index] != want[index] {
								test.Fatalf("position %d under B: got %q want %q", index, titles[index], want[index])
							}
						}
					},
				},
			...
			Steps: []Step{
				...
				{Args: []string{"task", "create", "Third", "parent=$0.short_id", "order=2.5"}},  // 3
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						mapped, ok := parsed.(map[string]any)
						if !ok {
							test.Fatalf("expected resequence response map, got %T", parsed)
						}
						if mapped["rewritten"] != float64(3) {
							test.Fatalf("expected rewritten=3, got %v", mapped["rewritten"])
						}
					},
					AssertText: func(test *testing.T, output string) {
						test.Helper()
						assertContains(test, output, "resequenced 3 tasks under parent")
					},
				},
				{
					Args: []string{"task", "get", "$1.short_id"},
		...
		},
		{
			...
			Steps: []Step{
				...
				{Args: []string{"task", "create", "Incoming", "parent=$0.short_id"}},                         // 3
				{
					...
					Assert: func(test *testing.T, result Result) {
						test.Helper()
						assertStderrContains(test, result, "no float64 midpoint")
						assertStderrContains(test, result, "parent")
					},
				},
			},
		},
			...
			Steps: []Step{
				...
				{Args: []string{"task", "create", "K5", "parent=$0.short_id", "order=5"}}, // 5
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						arr := jsonArray(test, parsed)
						if len(arr) != 3 {
							test.Fatalf("expected 3 tasks in range, got %d", len(arr))
						}
						titles := map[string]bool{}
						for _, item := range arr {
							titles[item.(map[string]any)["title"].(string)] = true
						}
						for _, want := range []string{"K2", "K3", "K4"} {
							if !titles[want] {
								test.Fatalf("expected %s in filtered set, got %v", want, titles)
							}
						}
						for _, unwanted := range []string{"K1", "K5"} {
							if titles[unwanted] {
								test.Fatalf("did not expect %s in filtered set, got %v", unwanted, titles)
							}
						}
					},
				},
			...
			Steps: []Step{
				...
				{Args: []string{"task", "create", "High prio", "parent=$0.short_id", "priority=3"}}, // 3
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						arr := jsonArray(test, parsed)
						if len(arr) != 1 {
							test.Fatalf("subtree: expected 1 root, got %d", len(arr))
						}
						root := arr[0].(map[string]any)
						children := root["children"].([]any)
						if len(children) != 3 {
							test.Fatalf("default subtree: expected 3 children, got %d", len(children))
						}
						titles := []string{
							children[0].(map[string]any)["title"].(string),
							children[1].(map[string]any)["title"].(string),
						...
						for index := range want {
							if titles[index] != want[index] {
								test.Fatalf("default sort position %d: got %q want %q", index, titles[index], want[index])
							}
						}
					},
				},
				{
					...
					AssertJSON: func(test *testing.T, parsed any) {
						...
						arr := jsonArray(test, parsed)
						if len(arr) != 1 {
							test.Fatalf("urgency subtree: expected 1 root, got %d", len(arr))
						}
						root := arr[0].(map[string]any)
						children := root["children"].([]any)
						if len(children) != 3 {
							test.Fatalf("urgency subtree: expected 3 children, got %d", len(children))
						}
						...
						if firstTitle != "High prio" {
							...
							test.Fatalf("urgency sort: expected High prio first, got %v", titles)
						}
					},
				},
			},
