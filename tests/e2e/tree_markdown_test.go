...
func shortIDFromCreate(test *testing.T, out string) string {
	test.Helper()
	fields := strings.Fields(strings.TrimSpace(out))
	if len(fields) == 0 {
		test.Fatalf("unexpected create output: %q", out)
	}
...
func TestTreeMarkdown_FullDialect(test *testing.T) {
	if binPath == "" {
	...
	milestoneID := shortIDFromCreate(test, mustRun(test, env,
		"task", "create", "v1 launch",
		"project=roadmap",
		`description="Ship the first public release."`,
	).Stdout)

	initiativeID := shortIDFromCreate(test, mustRun(test, env,
	...
	completedStoryID := shortIDFromCreate(test, mustRun(test, env,
	...
	activeStoryID := shortIDFromCreate(test, mustRun(test, env,
		"task", "create", "Sample data import",
		"project=roadmap",
		"parent="+initiativeID,
	).Stdout)

	csvLeafID := shortIDFromCreate(test, mustRun(test, env,
	...
	mustRun(test, env, "task", "modify", activeStoryID, "status=active")
	mustRun(test, env, "task", "modify", completedStoryID, "status=active")
	mustRun(test, env, "task", "modify", completedStoryID, "status=completed")
	...
	// per-task note.
	mustRun(test, env, "task", "annotate", milestoneID, "Initial scope ratified")
	mustRun(test, env, "note", "add", "caching strategy notes",
		"project=roadmap", "--player", "german",
	)
	mustRun(test, env, "note", "add", "retry needed",
		"project=roadmap", "--player", "german", "--task", csvLeafID,
	)

	out := mustRun(test, env, "task", "tree", "--format", "markdown").Stdout

	if !strings.HasPrefix(out, "# Roadmap\n") {
		test.Fatalf("expected H1 for project, got:\n%s", out)
	}
	if !strings.Contains(out, "> Roadmap-driven product backlog") {
		test.Fatalf("expected description blockquote, got:\n%s", out)
	}
	if !strings.Contains(out, "## v1 launch") {
		test.Fatalf("expected ## milestone heading, got:\n%s", out)
	}
	if !strings.Contains(out, "### Onboarding flow") {
		test.Fatalf("expected ### initiative heading, got:\n%s", out)
	}
	if !strings.Contains(out, "- [x] Welcome screen") {
		test.Fatalf("expected completed-story checkbox `- [x] Welcome screen`, got:\n%s", out)
	}
	if !strings.Contains(out, "- [ ] Sample data import status=active") {
		test.Fatalf("expected active-story bullet with status=active, got:\n%s", out)
	}
	if !strings.Contains(out, "  - [ ] Wire CSV parser") {
		test.Fatalf("expected indented leaf bullet under active story, got:\n%s", out)
	}
	if strings.Contains(out, "<!-- tusk: markdown body lands in phase 4 -->") {
		test.Fatalf("phase-3 placeholder must be gone, got:\n%s", out)
	}

	// Phase 5 additions: annotations and notes labels and bodies.
	if !strings.Contains(out, "**Annotations:**") {
		test.Fatalf("expected **Annotations:** label, got:\n%s", out)
	}
	if !strings.Contains(out, "**Notes:**") {
		test.Fatalf("expected **Notes:** label, got:\n%s", out)
	}
	if !strings.Contains(out, "Initial scope ratified") {
		test.Fatalf("expected milestone annotation body, got:\n%s", out)
	}
	if !strings.Contains(out, "caching strategy notes") {
		test.Fatalf("expected project-level note body, got:\n%s", out)
	}
	if !strings.Contains(out, "retry needed") {
		test.Fatalf("expected leaf-task note body, got:\n%s", out)
	}
	...
	filteredOut := mustRun(test, env,
		"task", "tree", "project=roadmap", "--format", "markdown",
	).Stdout
	if filteredOut != out {
		test.Fatalf("project=<name> filter should match bare invocation in single-project workspace.\nbare:\n%s\nfiltered:\n%s", out, filteredOut)
	}
}
