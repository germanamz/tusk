...
export async function fetchIndex(signal?: AbortSignal): Promise<IndexResponse> {
  return fetchJSON<IndexResponse>('/api/read/index', { signal })
}

export async function fetchNode(id: string, signal?: AbortSignal): Promise<NodeReadPayload> {
  return fetchJSON<NodeReadPayload>(`/api/read/node/${encodeId(id)}`, { signal })
}
...
export async function postSearch(req: SearchRequest, signal?: AbortSignal): Promise<SearchResponse> {
  const resp = await fetch('/api/read/search', {
    method: 'POST',
  ...
  if (!resp.ok) throw new Error(`/api/read/search: ${resp.status}`)

  return (await resp.json()) as SearchResponse
}

// RelatedOptions carries GET /api/read/related/{id...}'s optional query params.
...
export async function fetchRelated(
  ...
  const url = `/api/read/related/${encodeId(id)}${qs ? `?${qs}` : ''}`
