...
//   4. images resolved to /api/read/asset/... or turned into click-through links
...
function resolveAssetPath(nodeDir: string, src: string): string {
  ...
  return `/api/read/asset/${encodeId(parts.join('/'))}`
}
...
export async function hydrate(container: HTMLElement, ctx: RenderContext): Promise<void> {
  ...
  // Snapshot the teardown generation before the async mermaid pass. If a sweep
  // (a later navigation, or the view's unmount) runs while mermaid.run is
  // pending, the generation advances and the Panzoom instances built below no
  // longer belong to the live pass — they get torn down immediately rather than
  // pushed onto the swept array, where unmount could never reach them.
  const generation = teardownGeneration

  const mermaidBlocks = container.querySelectorAll<HTMLElement>('pre.mermaid')

  if (mermaidBlocks.length > 0) {
    // mermaid and the theme controller are both pulled in lazily here, next to
    // the only place that needs them: a node with no diagrams never loads
    // either. mermaid.initialize mutates global module state, so the theme is
    // read fresh on every hydrate pass — a theme flip re-renders the open node
    // (index.ts) and this picks up the new value. mermaid ships only 'neutral'
    // and 'dark' among its stock light/dark pair, so map day ops -> 'neutral'.
    const [{ default: mermaid }, { getResolvedTheme }] = await Promise.all([
      import('mermaid'),
      import('../theme/theme'),
    ])
    mermaid.initialize({
      ...
      theme: getResolvedTheme() === 'dark' ? 'dark' : 'neutral',
    })
    ...
    try {
      ...
      mermaidBlocks.forEach((block) => {
        ...
        if (generation === teardownGeneration) {
          activeZoomTeardowns.push(teardown)
        } else {
          // A sweep ran while mermaid.run was pending; this pass is superseded,
          // so destroy its Panzoom instance now instead of leaking it.
          teardown()
        }
      })
    } catch {
...
// teardownGeneration advances on every sweep so an in-flight hydrate can detect
// that its Panzoom instances were superseded before it finished (see hydrate).
...
// teardownActiveDiagramZooms is teardownPreviousDiagramZooms exposed for the
// view's unmount (index.ts): leaving the reader tears down the last node's
// Panzoom instances so none survive past the view that created them.
export function teardownActiveDiagramZooms(): void {
  teardownPreviousDiagramZooms()
}
