...
// wheel at medium saturation/lightness) chosen to read well on the dark graph
// canvas. These 12 categorical node hues are theme-independent by design — a
// node keeps its identifying color in both light and dark. Used by
// buildTypeColors to assign a hue per node type.
...
// Edge-kind colors, read from the theme's CSS custom properties at call time so
// they track light/dark. The fallback hexes match the dark palette and cover a
// stray read before the theme is stamped (e.g. a unit test with no stylesheet).
export function edgeKindColors(): Record<string, string> {
  return {
    direct: cssVar('--graph-edge-direct') || '#7aa2f7',
    derived: cssVar('--graph-edge-derived') || '#9d7cd8',
    structural: cssVar('--graph-edge-structural') || '#565f89',
  }
}

// Selection palette, likewise theme-driven and read at call time. The selected
// node burns to the theme's selected color; its incident edges glow the
// highlight hue; transient search pulses use the distinct pulse hue.
export const selectedColor = (): string => cssVar('--graph-selected') || '#ffffff'
export const highlightLinkColor = (): string => cssVar('--graph-highlight') || '#f5d76e'
export const pulseColor = (): string => cssVar('--graph-pulse') || '#f5a623'

// graphBackground is the canvas background that dimmed colors blend toward. Read
// from --graph-bg at call time and parsed into the [r,g,b] triple dimColor
// needs, so a theme flip re-tints every dimmed node.
function graphBackground(): [number, number, number] {
  return parseHexRgb(cssVar('--graph-bg')) ?? [0x0a, 0x0e, 0x15]
}

// parseHexRgb turns a #rrggbb string into its [r,g,b] byte triple, or null when
// the input is not a well-formed six-digit hex (e.g. the empty string a
// getComputedStyle read returns before the theme is applied).
