...
describe('encode', () => {
  ...
  describe('edgeKindColors', () => {
    it('reads a color for each edge kind from the theme CSS variables', () => {
      document.documentElement.style.setProperty('--graph-edge-direct', '#111111')
      document.documentElement.style.setProperty('--graph-edge-derived', '#222222')
      document.documentElement.style.setProperty('--graph-edge-structural', '#333333')
      const colors = edgeKindColors()
      expect(colors.direct).toBe('#111111')
      expect(colors.derived).toBe('#222222')
      expect(colors.structural).toBe('#333333')
      document.documentElement.style.removeProperty('--graph-edge-direct')
      document.documentElement.style.removeProperty('--graph-edge-derived')
      document.documentElement.style.removeProperty('--graph-edge-structural')
    })

    it('falls back to the dark palette when the variables are unset', () => {
      const colors = edgeKindColors()
      expect(colors.direct).toBe('#7aa2f7')
      expect(colors.derived).toBe('#9d7cd8')
      expect(colors.structural).toBe('#565f89')
    })
  })
  ...
  describe('dimColor', () => {
    // dimColor blends toward --graph-bg, read at call time. Pin it to the dark
    // graph background so the blend target is deterministic in jsdom (where no
    // stylesheet is loaded).
    beforeAll(() => {
      document.documentElement.style.setProperty('--graph-bg', '#0a0e15')
    })
    ...
    it('is identity at amount 0 and the background at amount 1', () => {
      ...
      expect(dimColor('#4f8cff', 1)).toBe('#0a0e15')
    })
