...
// Fallback background hex for the fleeting window before the theme is stamped
// (the browser always has tokens.css loaded and a data-theme by mount time).
const GRAPH_BG_FALLBACK = '#0a0e15'
const NODE_FALLBACK = '#888888'
...
export interface Scene {
  ...
  /** Re-read the theme's graph tokens (background, edge/selection colors) and
   *  repaint. Call on a light/dark flip; the caller follows with a setGraph
   *  re-render so node/link colors update too. */
  refreshTheme(): void
  /** Tear down the scene: disconnect the ResizeObserver and free the WebGL
   *  context so a re-mount starts clean. */
  dispose(): void
...
export function createScene(el: HTMLElement): Scene {
  ...
  const graph = new ForceGraph3D(el)
    ...
    .backgroundColor(cssVar('--graph-bg') || GRAPH_BG_FALLBACK)
    .nodeId('id')
  ...
  // Theme-derived colors, cached so the per-node/per-link accessors below don't
  // call getComputedStyle on every repaint. refreshTheme() re-reads these on a
  // light/dark flip; the edge-kind map, selection/highlight/pulse hues, and the
  // ungrouped-node fallback all follow the active theme.
  let edgeColors = edgeKindColors()
  let selColor = selectedColor()
  let highlightColor = highlightLinkColor()
  let pulseCol = pulseColor()
  let nodeFallback = cssVar('--graph-node-fallback') || NODE_FALLBACK
  ...
  const nodeColor = (node: any): string => {
    if (pulseIds.has(node.id)) return pulseCol
    if (node.id === selectedId) return selColor
    const base = importanceColor(groupColors.get(node.group) ?? nodeFallback, node.degree ?? 0, maxDegree)
  ...
  const linkColor = (link: any): string => {
    if (highlightLinks.has(link)) return highlightColor
    const kind: string = edgeColors[link.kind] ?? nodeFallback
  ...
  return {
    ...
    refreshTheme() {
      // Re-read every theme-driven color, re-tint the canvas, and repaint the
      // accessors. dimColor picks up the new --graph-bg on its own (it reads it
      // at call time), so the cached values here are the ones the accessors read
      // directly: the edge-kind map, selection/highlight/pulse hues, and the
      // ungrouped fallback.
      edgeColors = edgeKindColors()
      selColor = selectedColor()
      highlightColor = highlightLinkColor()
      pulseCol = pulseColor()
      nodeFallback = cssVar('--graph-node-fallback') || NODE_FALLBACK
      graph.backgroundColor(cssVar('--graph-bg') || GRAPH_BG_FALLBACK)
      refresh()
    },
    dispose() {
      resize.disconnect()
      // 3d-force-graph exposes _destructor() to free the renderer + WebGL
      // context; guard the call so a library rename can't wedge unmount.
      const destroy = (graph as unknown as { _destructor?: () => void })._destructor
      if (typeof destroy === 'function') destroy.call(graph)
    },
  }
}
